...
export function SearchBar() {
  ...
  const handleFilterOptions = () => {
    const filtered = searchOptions
      .filter((opt) =>
        opt?.text.toLowerCase().includes(inputText.toLowerCase()),
      )
      .map((opt) => opt?.text ?? "");
  ...
  return (
    <Box className="flex h-screen w-full flex-col items-center bg-white pt-[80px]">
      ...
      <Button
        variant="contained"
        className="w-[250px] bg-blue-700 normal-case text-white hover:bg-blue-800"
        sx={{
          paddingY: 1.2,
          fontWeight: "bold",
          fontSize: "14px",
        ...
        onClick={async () => {
          ...
          setSearchValueInferece(inputText);
                    ...
                    <Typography
                      ...
                      className={`flex w-[100px] flex-shrink-0 items-center justify-center rounded px-2 py-1 text-center font-semibold ${
                        claim.cleaned_veracity?.toLowerCase() === "true"
                          ? "border border-green-300 bg-green-200 text-green-900"
      ...
      {showProgressBar && (
        <Box
          sx={{ width: "100%", maxWidth: "800px", mt: 2, textAlign: "center" }}
        >
          ...
          {showText && (
            ...
            >
              Please wait for about 30 seconds, our model is analyzing the
              claim...
            </Typography>
          )}
        </Box>
      ...
      <Box className="mt-4 flex w-full max-w-[720px] flex-col items-start gap-2">
        ...
        >
          The United States is currently only contributing 15% of emissions, but
          we are the largest historical contributor (we have had the most
          emissions over time).
        </Button>
        <Button
          variant="outlined"
        ...
        >
          In September, New York City saw a similar story play out, as 7 inches
          of rain fell in 24 hours in some locations, submerging cars and city
          buses and shuttering rail travel., , Before July 2023, has Vermont
          *never* experienced intense rainfall or flash flooding?, * Before July
          2023, have some locations in New York City *never* had 7 inches of
          rainfall over a 24-hour period?, The consequences of storms are
          intensified in cities like New York, where storm drains and subway
          tunnels are in disrepair or were simply built for a more gentle
          climate..
        </Button>
        <Button
          variant="outlined"
        ...
        >
          The US has a little over 200 coal-fired power plants, and the number
          is shrinking.
        </Button>
      </Box>
